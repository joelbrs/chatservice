...
import (
	...
	"github.com/joelbrs/chatservice/internal/domain/gateway"
...
func NewChatCommpletionUseCase(chatGateway gateway.ChatGateway, openAiClient openai.Client) *ChatCompletionUseCase {
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO, stream chan ChatCompletionOutputDTO) (*ChatCompletionOutputDTO, error) {
	chat, err := uc.chatGateway.FindChatByID(ctx, input.ChatID)

	if err != nil {
		if err.Error() == "chat not found" {
			// create new chat (entities)
	...
	if err != nil {
		return nil, errors.New("error creating new user message: " + err.Error())
	}
